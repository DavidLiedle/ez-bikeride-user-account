<?php
/**
 * File containing the Gpx FieldType Type class.
 *
...
class Type extends FieldType
{
    ...
    public function __construct(GpxClientInterface $gpxClient)
    {
        $this->gpxClient = $gpxClient;
    }
    ...
    {
        // return preg_replace(
        //     '#^https?://gpx\.com/([^/]+)/status/([0-9]+)$#',
        //     '$1-$2',
        //     (string)$value->url
        // );
        return "gpxnamechangeme";
    }
    ...
    public function toPersistenceValue(SPIValue $value)
    {
        ...
        if ($value->contents === null) {
            $value->contents = $this->gpxClient->getEmbed($value->url);
        }
     ...
     *
     * @return \EzSystems\GpxFieldTypeBundle\eZ\Publish\FieldType\Gpx\Value
     */
    ...
    public function validateValidatorConfiguration($validatorConfiguration)
    {
        ...
        foreach ($validatorConfiguration as $validatorIdentifier => $constraints) {
            ...
            if ($validatorIdentifier !== 'GpxValueValidator') {
                $validationErrors[] = new ValidationError("Validator '$validatorIdentifier' is unknown");
                continue;
            }

            // Validate arguments from GpxValueValidator
                    ...
                    case 'authorList':
                        ...
                        foreach ($value as $authorName) {
                            if (!preg_match('/^[a-z0-9_]{1,15}$/i', $authorName)) {
                                $validationErrors[] = new ValidationError('Invalid gpx username');
                            }
                        }
                        break;
                    default:
    ...
    public function validate(FieldDefinition $fieldDefinition, SPIValue $fieldValue)
    {
        ...
        // Gpx Url validation
        if (!preg_match('#^https?://gpx.com/([^/]+)/status/[0-9]+$#', $fieldValue->url, $m)) {
            $errors[] = new ValidationError(
                'Invalid gpx status url %url%',
                null,
                ['%url%' => $fieldValue->url]
            );
        ...
        if (!$this->isAuthorApproved($author, $validatorConfiguration)) {
            $errors[] = new ValidationError(
                'Gpx user %user% is not in the approved author list',
                null,
                ['%user%' => $m[1]]
            );
        }
    ...
    {
        return !isset($validatorConfiguration['GpxValueValidator'])
            || empty($validatorConfiguration['GpxValueValidator']['authorList'])
            || in_array($author, $validatorConfiguration['GpxValueValidator']['authorList']);
    }
}
